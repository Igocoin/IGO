This directory contains the IgocoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Igocoin UI.
...
ClientModel is used by the main application `igocoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/igocoin/igocoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a igocoin:... link from a browser).
...
* UI elements like IgocoinAmountField, which inherit from QWidget.
* `igocoinstrings.cpp`: automatically generated
* `igocoinunits.(h/cpp)`: IGO / mIGO / etc handling
* `callback.h`
...
* don't change `local/igocoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "igocoin-qt" under "Run", which is where you can also add command line arguments)
