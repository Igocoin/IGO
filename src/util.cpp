...
const char * const IGOCOIN_CONF_FILENAME = "igocoin.conf";
const char * const IGOCOIN_PID_FILENAME = "igocoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "igocoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Igocoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Igocoin
    // Mac: ~/Library/Application Support/Igocoin
    // Unix: ~/.igocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Igocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Igocoin";
#else
    // Unix
    return pathRet / ".igocoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", IGOCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", IGOCOIN_PID_FILENAME)));
}
