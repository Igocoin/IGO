...
###   igocoincore.org and igocoin.org and compares them.
...
WORKINGDIR="/tmp/igocoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://igocoincore.org"
HOST2="https://igocoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="igocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'igocoin-',
...
if [ $? -ne 0 ]; then
   echo "igocoin.org failed to provide signature file, but igocoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "igocoin.org and igocoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Igocoin Core binary release signing key installed?"
   fi
